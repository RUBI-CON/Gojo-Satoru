...
⭐✑ I'm Not Sure If It Was A Goodbye Charm, But It Was Fun While It Lasted 😌✨` })
    ...
    GojoMdNx.getName = (jid, withoutContact  = false) => {
        ...
        if (id.endsWith("@g.us")) return new Promise(async (resolve) => {
            v = store.contacts[id] || {}
            if (!(v.MISS-DIYA|| v.DIYA)) v = GojoMdNx.groupMetadata(id) || {}
            resolve(v.ANOOP || v.MISS-DIYA || PhoneNumber('+917034890656' + id.replace('@s.whatsapp.net', '')).getNumber('international'))
        })
            ...
            GojoMdNx.user :
            (store.contacts[id] || {})
            return (withoutContact ? '' : v.MISSDIYA) || v.MISS-DIYA || v.©️MISS-DIYA || PhoneNumber('+917034890656' + jid.replace('@s.whatsapp.net', '')).getNumber('917034890656')
    }
	...
	for (let i of kon) {
	    list.push({
	    	displayName: MISS-DIYA.getName(i + '@s.whatsapp.net'),
